...
DEFAULT_SOURCE_IMAGES: Dict[str, str] = {
    ...
    "Anthropic News": "https://images.unsplash.com/photo-1620712943543-bcc4688e7485?auto=format&fit=crop&w=800&q=80",
...
"""
        ...
        b0 = bullets[0] if len(bullets) > 0 else f"'{title_kr}' 관련 신규 파이프라인 및 아키텍처 호환성을 검토하세요."
...
"""
    ...
    # API 키가 정식 유효한 경우에만 LLM 호출 시도
    if not (client.api_key and len(client.api_key) > 10 and client.api_key != "front"):
        raw_t = str(raw.get("title") or "AI 기술 피드 리포트")
        title_kr, summary_bullets, blog_summary = auto_translate_and_format(raw_t, str(raw.get("summary") or ""), str(raw.get("source_name") or "AI Feed"), raw.get("category", "빅테크 공식"))
        ...
        return NewsArticle(
            ...
            actionable_insight=ActionableInsight(
                developer=summary_bullets[0] if len(summary_bullets) > 0 else None,
                pm=summary_bullets[1] if len(summary_bullets) > 1 else None,
                business=summary_bullets[2] if len(summary_bullets) > 2 else None
            ),
            impact_score=85,
...
def cluster_raw_articles_by_topic(raw_articles: List[Dict[str, Any]]) -> List[List[Dict[str, Any]]]:
    ...
    TOPIC_PATTERNS = {
        "fine_tuning": ["fine-tune", "fine-tuning", "파인튜닝", "튜닝"],
        "agent": ["agent", "agentic", "swe-bench", "자율", "에이전트", "computer use"],
        "moe_reasoning": ["moe", "reasoning", "mcts", "r1", "deepseek", "추론"],
        "multimodal_vision": ["vision", "multimodal", "tactus", "image", "멀티모달", "비전"],
        "enterprise_tco": ["tco", "cost", "hosting", "on-prem", "vllm", "호스팅", "비용"],
        "security_eval": ["security", "safety", "guardrail", "보안", "가드레일"]
    }
...
"""
    try:
        ...
        return NewsArticle(
            ...
            title=data.get("title_kr", cluster[0].get("title", "AI 융합 리포트")),
    ...
    try:
        ...
        try:
            ...
            for cluster in clusters:
                synth_article = synthesize_article_cluster(cluster)
                if synth_article and synth_article.title not in seen_titles:
                    seen_titles.add(synth_article.title)
                    articles.append(synth_article)
        except Exception as e:
            print(f"[NewsBatch Warning] Cluster synthesis pipeline notice: {e}")
